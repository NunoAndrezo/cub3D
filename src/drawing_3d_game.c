...
/*   Updated: 2026/01/01 16:44:39 by joaoleote        ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		int hit_side, int column_index)
{
	...
	x = (int)((float)column_index / (float)game->ray.num_rays * 
		game->image.width);
	if (x < 0 || x >= game->image.width)
		return;
	calculate_projection(game, angle, best_dist, &draw_start, &draw_end);
...
void	calculate_projection(t_game *g, float ray_angle, float dist, 
							int *start, int *end)
{
	float	corrected;
	float	player_rad;
	...
	if (corrected < 0.0001f)
		corrected = 0.0001f;
	line_h = (int)((float)ONE_TILE_SIDE
			* g->image.height / corrected);
