...
/*   Updated: 2026/01/06 03:21:51 by joaoleote        ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main_loop(t_game *game)
{
	...
	uint64_t		frame_end;
	uint64_t		elapsed;
	const uint64_t	target_frame_ms = 16;
	uint64_t		frame_time;
...
static int	game_loop(t_game *game)
{
	...
	if (game->bg_image.img_pixels_ptr && game->image.img_pixels_ptr)
		ft_memcpy(game->image.img_pixels_ptr,
			game->bg_image.img_pixels_ptr, row_bytes);
	draw_player(game);
	lets_see_them_rays(game);
	mlx_put_image_to_window(game->mlx_struct, game->win_struct,
